...
function multiplicarNumeroDesconocidoPorCinco(objetoMisterioso) {
   ...
   // Tu código:
   let multiplyFive = objetoMisterioso.numeroMisterioso * 5;
   return multiplyFive;
}
...
function sumarLikesDeUsuario(objetoUsuario) {
   ...
   let likesInArr = [];
   let sumaLikes = 0;
   let publis = objetoUsuario.posts;

   publis.forEach((post) => {
      likesInArr.push(post.likes)
   })
   
   for(i = 0; i < likesInArr.length; i++) {
      sumaLikes += likesInArr[i];
   }
   return sumaLikes;
}
